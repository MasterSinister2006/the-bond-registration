...
## No-Mongo deployment
This build uses Google Sheets as the persistent registration sink. MongoDB and the reminder worker are removed from the backend startup path. Registrations are written to the configured Google Sheet immediately; proof/approval state is kept in process memory while the backend is running. Email and Calendar integrations remain optional and disabled until configured.
