...
export const apiGetWithHeaders = <T>(path: string, headers: Record<string, string>) =>
  request<T>("GET", path, undefined, headers);
export const apiPostWithHeaders = <T>(
  path: string,
  body: JsonBody | undefined,
  headers: Record<string, string>,
) => request<T>("POST", path, body ?? null, headers);
...
// FastAPI error bodies are either {detail: "some message"} (a raised HTTPException) or
// {detail: [{msg: "..."}, ...]} (a 422 validation failure). Pull the human-readable part
// out so a toast can show the organiser's actual reason instead of a generic guess.
