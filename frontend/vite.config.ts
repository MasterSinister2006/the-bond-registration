...
// Fails open, same as the overlay below: this Emergent-only package is not on the
// public npm registry, so it is absent outside the Emergent pod (Vercel included).
// A missing package must degrade to "no visual edits", never break the build.
async function loadVisualEdits() {
  ...
  try {
    ...
    console.warn("[visual-edits] plugin not available; skipping:", e instanceof Error ? e.message : e);
    return null;
  }
}
